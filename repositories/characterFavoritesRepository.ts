...
export function createCharacterFavoritesRepository(
  client: SupabaseClient,
): CharacterFavoritesRepository {
  return {
    async load() {
      const { data, error } = await client
        .rpc("get_character_favorites")
      ...
      if (
        ...
        data.counts === null ||
        (data.favorite !== null && typeof data.favorite !== "string")
      )
      ...
      const request =
        characterId === null
          ...
          : client
              ...
              .upsert({ user_id: userId, character_id: characterId }, { onConflict: "user_id" });
