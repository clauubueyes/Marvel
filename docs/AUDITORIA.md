...
Ordenadas por prioridad. Cada ítem indica _qué_, _dónde_ y _cómo_.
...
- **Problema**: toda la UI se resuelve por cascada de `@import` y selectores globales
  (páginas con clases largas en una línea). El CSS crece y no hay aislamiento ni _scoping_;
...
Hoja de ruta priorizada. Cada idea incluye el _porqué_ y la _aproximación técnica_.
