...
test("shared loading deduplicates all cards; guests cannot write", async () => {
  const response = deferred<FavoritesData>();
  let loads = 0;
  let saves = 0;
  const store = new CharacterFavoritesStore({
    load: () => {
      loads++;
  ...
  response.resolve({ counts: { spider: 12 }, favorite: null });
  await loading;
  await store.toggle("spider");
  store.ensureLoaded();
...
test("choose, move and remove are immediate; rapid clicks cannot create extra writes", async () => {
  ...
  await store.toggle("spider");
  await store.toggle("iron");
  assert.equal(writes, 1);
  assert.equal(store.getSnapshot().counts.spider, 3);
  store.setUser("alice");
  assert.equal(store.getSnapshot().pending, true);
  saved.resolve();
  await first;
  saved = deferred<void>();
  const move = store.toggle("iron");
  assert.equal(store.getSnapshot().counts.spider, 2);
  assert.equal(store.getSnapshot().counts.iron, 1);
  saved.resolve();
  await move;
  saved = deferred<void>();
  const remove = store.toggle("iron");
  assert.equal(store.getSnapshot().favorite, null);
  assert.equal(store.getSnapshot().counts.iron, 0);
  saved.resolve();
  await remove;
...
test("failure rolls back both counts and selection and requires reconciliation", async () => {
  const store = new CharacterFavoritesStore({
    load: async () => ({ counts: { spider: 1 }, favorite: "spider" }),
    save: async () => {
      throw new Error();
    },
  });
...
test("late saves and loads cannot restore a logged-out user's private state", async () => {
  ...
  const store = new CharacterFavoritesStore({
    load: async () => ({ counts: {}, favorite: null }),
    save: () => saved.promise,
  });
  store.setUser("alice");
  await store.load();
  const saving = store.toggle("spider");
  store.setUser(null);
  assert.equal(store.getSnapshot().favorite, null);
  ...
  await saving;
  assert.equal(store.getSnapshot().favorite, null);
  const old = deferred<FavoritesData>();
  let calls = 0;
  const other = new CharacterFavoritesStore({
    load: () =>
      ++calls === 1 ? old.promise : Promise.resolve({ counts: { iron: 1 }, favorite: "iron" }),
    save: async () => {},
  });
  other.setUser("alice");
  ...
  other.setUser("bob");
  await other.load();
  old.resolve({ counts: { spider: 1 }, favorite: "spider" });
  await loading;
...
test("failed reads retry without loops; a confirmed write is not rolled back when its refresh fails", async () => {
  ...
  await store.toggle("spider");
  assert.equal(store.getSnapshot().favorite, "spider");
  assert.equal(store.getSnapshot().counts.spider, 1);
  assert.equal(store.getSnapshot().ready, false);
