...
import {
  getCharacterSpoilerRequirements,
  getCharacterProgressTitleIds,
} from "@/utils/characterSpoilers";
...
test("explicit spoiler opt-out reveals without changing progress, only after initialization", () => {
  ...
  assert.equal(
    canRevealSpoiler(requirement, { ready: false, watched, allowSpoilers: true }),
    false,
  );
...
test("Iron Man and Iron Man 2 unlock only the origin; later works never imply earlier ones", () => {
  ...
  assert.deepEqual(
    storyData.iron.map((chapter) => canRevealSpoiler(chapter.spoiler, progress)),
    [true, false, false, false],
  );
  assert.deepEqual(
    storyData.iron.map((chapter) =>
      canRevealSpoiler(chapter.spoiler, { ready: true, watched: new Set(["vengadores-endgame"]) }),
    ),
...
test("loading, unknown requirements and incomplete multiple requirements stay locked", () => {
  ...
  assert.equal(
    canRevealSpoiler({ allOf: ["iron-man", "iron-man-2"] }, { ready: true, watched }),
    false,
  );
  assert.equal(
    canRevealSpoiler(
      { allOf: ["iron-man", "iron-man-2"] },
      { ready: true, watched: new Set(["iron-man", "iron-man-2"]) },
    ),
    true,
  );
...
test("locked data is replaced completely, including images, labels and nested information", () => {
  ...
  assert.deepEqual(
    protectContent<object>(
      content,
      { allOf: ["iron-man"] },
      { ready: false, watched: new Set() },
      replacement,
    ),
    replacement,
  );
  assert.equal(
    protectContent<object>(
      content,
      { allOf: ["iron-man"] },
      { ready: true, watched: new Set(["iron-man"]) },
      replacement,
    ),
    content,
  );
...
test("progress hint reports watched works without revealing which are missing", () => {
  ...
  assert.deepEqual(
    spoilerProgressHint({ allOf: ["iron-man", "iron-man-2", "vengadores-endgame"] }, progress),
    { watched: 1, required: 3 },
  );
  assert.deepEqual(spoilerProgressHint({ allOf: ["iron-man"] }, progress), null);
  assert.deepEqual(
    spoilerProgressHint(
      { allOf: ["iron-man", "iron-man-2", "vengadores-endgame"] },
      { ready: true, watched: new Set(["iron-man", "iron-man-2", "vengadores-endgame"]) },
    ),
    null,
  );
  ...
  assert.deepEqual(
    spoilerProgressHint(
      { allOf: ["iron-man", "iron-man-2"] },
      { ready: false, watched: new Set(["iron-man"]) },
    ),
    { watched: 1, required: 2 },
  );
  assert.deepEqual(
    spoilerProgressHint(
      { allOf: ["iron-man"] },
      { ready: true, watched: new Set(), allowSpoilers: true },
    ),
    null,
  );
});
...
test("every declared spoiler requirement references real catalog IDs", () => {
  ...
  for (const requirement of requirements) {
    ...
    for (const id of requirement.allOf)
      assert.ok(valid.has(id), `Unknown spoiler requirement: ${id}`);
  }
...
test("every character can unlock every protected field using their account title selection", () => {
  for (const character of characters) {
    ...
    for (const requirement of getCharacterSpoilerRequirements(character)) {
      ...
      assert.equal(
        canRevealSpoiler(requirement, { ready: true, watched: new Set() }),
        false,
        character.id,
      );
      for (const id of requirement.allOf) {
        ...
        missing.delete(id);
        assert.equal(
          canRevealSpoiler(requirement, { ready: true, watched: missing }),
          false,
          `${character.id} without ${id}`,
        );
      }
    }
  }
...
test("the build rejects missing, empty, unknown and misaligned spoiler requirements", () => {
  ...
  for (const spoiler of [undefined, { allOf: [] }, { allOf: ["unknown-title"] }]) {
    const changed = {
      ...original,
      story: original.story.map((chapter, index) =>
        index === 0 ? { ...chapter, spoiler } : chapter,
      ),
    };
  ...
  assert.ok(
    validateCharacterSpoilers(
      [{ ...original, facts: [...original.facts, original.facts[0]] }],
      valid,
    ).length,
  );
});
...
test("flashbacks and mixed acts use revealed works, not chronological years or appearances", () => {
  ...
  assert.equal(
    canRevealSpoiler(storyData.rocket[0].spoiler, progress(["guardianes-de-la-galaxia"])),
    false,
  );
  assert.equal(
    canRevealSpoiler(storyData.rocket[1].spoiler, progress(["guardianes-de-la-galaxia"])),
    true,
  );
  assert.equal(
    canRevealSpoiler(storyData.panther[2].spoiler, progress(["vengadores-infinity-war"])),
    false,
  );
  assert.equal(
    canRevealSpoiler(
      storyData.panther[2].spoiler,
      progress(["vengadores-infinity-war", "vengadores-endgame"]),
    ),
    true,
  );
  assert.equal(
    canRevealSpoiler(storyData["kate-bishop"][0].spoiler, progress(["los-vengadores"])),
    false,
  );
  assert.equal(
    canRevealSpoiler(storyData["kate-bishop"][0].spoiler, progress(["ojo-de-halcon"])),
    true,
  );
  assert.ok(
    getCharacterProgressTitleIds(characters.find(({ id }) => id === "captain-america")!).includes(
...
test("Captain America unlocks only the watched film and unreviewed character stories stay locked", () => {
  ...
  assert.deepEqual(
    storyData["captain-america"].map(({ spoiler }) => canRevealSpoiler(spoiler, progress)),
    [true, true, false, false],
  );
  for (const character of characters) {
    for (const chapter of character.story) {
      assert.equal(
        canRevealSpoiler(chapter.spoiler ?? UNREVIEWED_SPOILER, {
          ready: true,
          watched: new Set(),
        }),
        false,
  ...
  assert.equal(
    canRevealSpoiler(UNREVIEWED_SPOILER, {
      ready: true,
      watched: new Set(mcuCatalog.map(({ slug }) => slug)),
    }),
    false,
  );
  assert.equal(
    canRevealSpoiler(UNREVIEWED_SPOILER, { ready: true, watched: new Set(), allowSpoilers: true }),
    true,
  );
});
