...
export function EntityDossier({ entity }: { entity: MCUEntity }) {
  ...
  return <main className="entity-profile" style={{ "--accent": entity.color, "--entity-accent": entity.color, "--accent-2": "#4f6b28" } as React.CSSProperties}>
    ...
    <section className="entity-profile-hero">
      {imageTitle && <div className="entity-profile-art" aria-hidden="true"><Image src={`/api/title-image?title=${encodeURIComponent(imageTitle.title)}&type=${encodeURIComponent(imageTitle.type)}`} alt="" fill priority sizes="60vw" unoptimized /></div>}
    ...
    {relatedCharacters.length > 0 && <section className="entity-characters profile-section"><header><p className="section-label">04 / PERSONAJES</p><h2>QUIÉN ESTÁ<br /><em>IMPLICADO</em></h2></header><div>{relatedCharacters.map((character) => <Link href={`/personajes/${character.id}`} key={character.id}><Image src={character.image} alt="" fill sizes="(max-width: 650px) 50vw, 20vw" unoptimized /><span>{character.role}</span><h3>{character.name}</h3></Link>)}</div></section>}
  </main>;
}
