...
export function TitleDirectory({ titles }: Props) {
  ...
  const phases = useMemo(() => [...new Set(titles.map(({ phase: value }) => value))].sort(), [titles]);
  const visible = useMemo(() => titles
    .filter((title) =>
      (type === "TODOS" || title.type === type) &&
      (phase === "TODAS" || title.phase === phase) &&
      (saga === "TODAS" || getSaga(title.phase) === saga) &&
      (continuity === "TODAS" || title.continuity === continuity))
    .sort((left, right) => {
  ...
  const reset = () => { setType("TODOS"); setPhase("TODAS"); setSaga("TODAS"); setContinuity("TODAS"); setOrderMode("NARRATIVO"); setSortMode("ORDEN"); };

  return <>
    <section className="title-filters" aria-label="Filtros del catálogo de títulos">
      <div className="title-filter-types"><span>FORMATO</span>{["TODOS", "PELÍCULA", "SERIE", "ESPECIAL", "ONE-SHOT"].map((value) => <button key={value} className={type === value ? "active" : ""} onClick={() => setType(value)}>{value}</button>)}</div>
      ...
      <label><span>SAGA</span><select value={saga} onChange={(event) => setSaga(event.target.value)}><option>TODAS</option><option>SAGA DEL INFINITO</option><option>SAGA DEL MULTIVERSO</option><option>OTRAS HISTORIAS</option></select></label>
      <label><span>CONTINUIDAD</span><select value={continuity} onChange={(event) => setContinuity(event.target.value)}><option>TODAS</option><option>SAGA PRINCIPAL</option><option>MARVEL TELEVISION</option><option>MULTIVERSO</option></select></label>
      ...
      <p><strong>{visible.length}</strong> RESULTADOS</p>
      <button className="filter-reset" onClick={reset}>LIMPIAR</button>
      <button className={`view-toggle ${compact ? "active" : ""}`} onClick={() => setCompact((value) => !value)} aria-pressed={compact}>{compact ? "VISTA AMPLIA" : "VISTA COMPACTA"}</button>
    </section>

    <section className={`titles-directory ${compact ? "is-compact" : ""}`} aria-label="Archivo de títulos" aria-live="polite">
      {visible.map((title) => <Link className="title-directory-card" href={`/titulos/${title.slug}`} key={title.slug} data-reveal>
        <span>{String(title.order).padStart(2, "0")}</span>
        <div>
          <small>{title.period} · {title.type} · {title.coverage}</small>
          <h2>{title.title}</h2>
          <p>{title.event}</p>
          {title.routes.length > 0 && <div className="title-route-tags" aria-label="Recorridos relacionados">{title.routes.map((route) => <b key={route.slug}>{route.name}</b>)}</div>}
        </div>
        <aside><b>{title.phase}</b><small>{getSaga(title.phase)}</small><small>{title.continuity}</small><small>{title.releaseDateISO.slice(0, 4)}</small></aside>
        <i>↗</i>
      </Link>)}
      {!visible.length && <div className="title-directory-empty"><strong>SIN COINCIDENCIAS</strong><p>Prueba otra combinación de filtros.</p><button onClick={reset}>MOSTRAR TODO</button></div>}
    </section>
  </>;
}
