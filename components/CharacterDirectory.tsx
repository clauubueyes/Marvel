...
export function CharacterDirectory({ characters }: Props) {
  ...
  return <>
    ...
    <section className="characters-directory" aria-label="Catálogo de personajes" aria-live="polite">
      {visible.map((character, index) => <Link href={`/personajes/${character.id}`} className="character-directory-card" key={character.id} data-reveal style={{ "--card-accent": character.color, "--delay": `${index * 40}ms` } as React.CSSProperties}>
