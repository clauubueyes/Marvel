...
async function mockSupabase(page: Page) {
  ...
  await page.route("https://*.supabase.co/**", async (route) => {
    ...
    if (url.pathname.endsWith("/signup")) {
      ...
      return json({
        user: { id: "alice", email: "alice@example.com", identities: [] },
        session: null,
      });
    }
    if (url.pathname.endsWith("/user")) {
      ...
      return json({
        ...
        aud: "authenticated",
        role: "authenticated",
        user_metadata: preferences.get(id) ?? {},
      });
    }
...
test("account dashboard fits mobile and desktop and reloads progress from another device", async ({
  page,
}, testInfo) => {
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    await expect(page.getByRole("button", { name: "CERRAR SESIÓN", exact: true })).toBeVisible();
    await expect(
      page.getByRole("checkbox", { name: "Evitar spoilers", exact: true }),
    ).toBeVisible();
...
test("spoiler-free accounts protect Captain America and unreviewed character stories", async ({
  page,
}) => {
  const mock = await mockSupabase(page);
  ...
  await expect(page.locator(".story-card:not([data-next-watch])").first()).toContainText(
    "El hombre que no paraba",
  );
  await expect(page.locator(".story-card:not([data-next-watch])")).toHaveCount(2);
  await expect(page.locator(".story-cinema")).not.toContainText(
    /El final del baile|Devuelve las Gemas|agencia está tomada/,
  );
  await expect(page.locator(".intro-copy")).not.toContainText("INACTIVO");
  mock.progress.get("alice")!.set("vengadores-endgame", true);
  await page.reload();
  await expect(page.locator(".story-card:not([data-next-watch])").last()).toContainText(
    "El final del baile",
  );
  await expect(page.locator(".story-card:not([data-next-watch])")).toHaveCount(3);
});

test("registration is compact and spoiler preference persists independently of watched works", async ({
  page,
}) => {
  const mock = await mockSupabase(page);
  await page.goto("/cuenta");
  ...
  await preference.click();
  await expect(
    page.getByRole("alert").filter({ hasText: "No se pudo guardar tu preferencia" }),
  ).toBeVisible();
  await expect(preference).toBeChecked();
  await page.goto("/personajes/iron");
  await expect(page.locator(".story-card:not([data-next-watch])")).toHaveCount(0);
...
test("guest, registration, session refresh, watch/unwatch, rollback and account isolation", async ({
  page,
}) => {
  const mock = await mockSupabase(page);
...
test("intro runs once per browser and contact preserves the current page", async ({ page }) => {
  ...
  await page.reload();
  await expect(page.getByRole("dialog", { name: "Introducción de NEXUS" })).toHaveCount(0);
  await expect(page.getByRole("link", { name: "CONTACTO", exact: true })).toHaveAttribute(
    "target",
    "_blank",
  );
});

test("spoiler progress: catalog changes persist, Iron Man reveals only watched acts and logout locks them", async ({
  page,
}) => {
  const mock = await mockSupabase(page);
  ...
  const titleToggle = (title: string) =>
    page
      .locator(".title-directory-row")
      .filter({ has: page.getByRole("heading", { name: title, exact: true }) })
      .locator(".title-watch-toggle");
  await titleToggle("Iron Man").click();
  await titleToggle("Iron Man 2").click();
  await titleToggle("Iron Man 3").click();
  await expect
    .poll(() => [...(mock.progress.get("alice")?.values() ?? [])].filter(Boolean).length)
    .toBe(3);
  await titleToggle("Iron Man 3").click();
  ...
  page.on("request", (request) => {
    if (request.resourceType() === "image") requestedImages.push(decodeURIComponent(request.url()));
  });
  await page.goto("/personajes/iron");
  const story = page.getByRole("region", { name: "Historia de IRON MAN", exact: true });
  await expect(story.locator(".story-card:not([data-next-watch])").first()).toContainText(
    "Nacer en una cueva",
  );
  await expect(story.locator(".story-card:not([data-next-watch])")).toHaveCount(1);
  await expect(story).not.toContainText(
    /Salvar la ciudad|El precio de la verdad|Enfrentar al Titán Loco|2012|2016|2023/,
  );
  ...
  await expect(page.locator(".profile-facts")).not.toContainText(
    /3000|despedida|batalla definitiva/,
  );
  await expect(page.locator(".screen-moment img, .screen-moment iframe")).toHaveCount(0);
  expect(
    requestedImages.filter((url) => /history\/iron\/acto-[234]|moments\/iron/.test(url)),
  ).toEqual([]);
  ...
  await page.goto("/personajes/iron");
  await expect(story.locator(".story-card:not([data-next-watch])").nth(1)).toContainText(
    "Salvar la ciudad",
  );
...
test("spoiler rendering fails closed before hydration and updates guest progress without reloading", async ({
  page,
  browser,
  baseURL,
}) => {
