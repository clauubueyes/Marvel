...
test("favorites: guest totals, login, choose, move, remove, refresh, rollback and account isolation", async ({
  page,
}) => {
  ...
  const card = (id: string) =>
    page
      .locator(".character-directory-card")
      .filter({ has: page.locator(`a.character-card-link[href='/personajes/${id}']`) });
  ...
  await expect(fans("spider")).toHaveText("0 fans");
  await expect(fans("iron")).toHaveText("2 fans");
  ...
  await expect(favorite("spider")).toHaveAttribute("aria-pressed", "true");
  fail = false;
  await card("iron").getByText("REINTENTAR", { exact: true }).click();
  await expect(fans("spider")).toHaveText("1 fan");
  await expect(fans("iron")).toHaveText("1 fan");
  ...
  await page.goto("/personajes");
  await expect(favorite("spider")).toHaveAttribute("aria-pressed", "false");
  await favorite("iron").click();
  await expect(fans("iron")).toHaveText("2 fans");
  await expect.poll(() => votes.get("bob")).toBe("iron");
  expect(votes.get("alice")).toBe("spider");
