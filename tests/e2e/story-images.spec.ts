...
test("Iron Man and Avengers images remain visible before the pending Iron Man 2 trailer", async ({
  page,
}, testInfo) => {
  ...
  for (const index of [0, 1]) {
    ...
    expect(
      actual.equals(withoutPreview),
      `Act ${index + 1}: inactive trailer must not cover the image`,
    ).toBe(true);
