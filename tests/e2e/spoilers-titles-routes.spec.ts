...
test("titles dossier: summaries, post-credits, cast, connections and watch order stay locked until the title is watched", async ({
  page,
}) => {
  ...
  const disclosure = page.locator(".spoiler-disclosure button", {
    hasText: "RESUMEN CON SPOILERS",
  });
  ...
  for (const summary of summaries)
    await expect(page.locator(".context-nodes")).not.toContainText(summary);
  await expect(page.locator(".title-watch-columns .spoiler-notice")).toHaveCount(1);
  for (const after of afterTitles)
    await expect(page.locator(".title-watch")).not.toContainText(after);
  ...
  for (const summary of summaries)
    await expect(page.locator(".context-nodes").first()).toContainText(summary);
...
test("viewing route: a step spoiler stays locked until its title is watched", async ({ page }) => {
  ...
  const step = (titleId: string) =>
    page.locator(`.viewing-route-list li`, { has: page.locator(`a[href="/titulos/${titleId}"]`) });
  await expect(step(first.titleId).locator(".route-spoiler")).toContainText(
    "para revelar su spoiler",
  );
  ...
  await expect(step(second.titleId).locator(".route-spoiler")).toContainText(
    "para revelar su spoiler",
  );
