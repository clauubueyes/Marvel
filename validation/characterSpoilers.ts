...
export function validateCharacterSpoilers(
  ...
  for (const character of characters) {
    if (!character.story.length)
      errors.push(`${character.id}: historia sin requisitos de spoilers.`);
    if (
    ...
    for (const requirement of getCharacterSpoilerRequirements(character)) {
      if (!requirement?.allOf.length)
        errors.push(`${character.id}: contenido sensible sin obras de desbloqueo.`);
      else
        for (const id of requirement.allOf) {
          if (!titleIds.has(id))
            errors.push(`${character.id}: requisito de spoiler desconocido (${id}).`);
        }
    }
  }
  return errors;
