...
export function validateProgressRelations(
  ...
  for (const route of routes) {
    if (!route.steps.length)
      errors.push(`${route.name}: la ruta no tiene pasos y su requisito queda vacío.`);
    const seen = new Set<string>();
    for (const step of route.steps) {
      if (!valid.has(step.titleId))
        errors.push(`${route.name}: el paso ${step.titleId} no existe en el catálogo.`);
      if (seen.has(step.titleId))
        errors.push(
          `${route.name}: el paso ${step.titleId} se repite, el desbloqueo debería ser suficiente.`,
        );
      seen.add(step.titleId);
      if (!step.spoiler.trim())
        errors.push(`${route.name}: el paso ${step.titleId} no tiene spoiler que desbloquear.`);
    }
  }
  ...
  for (const entity of entities) {
    if (!entity.titleIds.length)
      errors.push(
        `${entity.name}: la conexión no referencia ningún título y queda bloqueada para siempre.`,
      );
  }
