...
export function TitleCredits({ title, details }: { title: TitleDossier; details: TitleDetails }) {
  return (
    <section className="title-credits profile-section">
      <header data-reveal>
        <p className="section-label">02 / FICHA TÉCNICA</p>
        <h2>
          QUIÉN LE DIO
          <br />
          <em>FORMA</em>
      ...
      <div className="title-credit-columns" data-reveal>
        <article>
          <span>DIRECCIÓN</span>
          {details.directors.length ? (
            details.directors.map((name) => <b key={name}>{name}</b>)
          ) : (
            <b>POR CONFIRMAR</b>
          )}
        </article>
        <article>
          ...
          {details.writers.length ? (
            details.writers.map((name) => <b key={name}>{name}</b>)
          ) : (
            <b>POR CONFIRMAR</b>
          )}
        </article>
