...
export function TitleConnections({
  entities,
  requirement,
}: {
  entities: MCUEntity[];
  ...
  return (
    <section className="context-nodes profile-section">
      <header>
        <p className="section-label">03 / CONEXIONES</p>
        <h2>
          MÁS ALLÁ
          <br />
          <em>DEL TÍTULO</em>
      ...
      <div>
        {entities.map((entity) => (
          <Link
            ...
            style={{ "--node-accent": entity.color } as React.CSSProperties}
          >
            <span>{entity.kind}</span>
            <strong>{entity.symbol}</strong>
            <h3>{entity.name}</h3>
            <p>
              <SpoilerText text={entity.summary} requirement={requirement} />
            </p>
            <b>EXPLORAR ↗</b>
          </Link>
        ))}
