...
export function TitleEventDossier({
  title,
  details,
  requirement = titleDossierRequirement(title.slug),
}: {
  title: TitleDossier;
    ...
    <section className="title-dossier profile-section">
      ...
      <div className="title-dossier-event" data-reveal>
        <ProgressSpoilerGate requirement={requirement}>
          <SpoilerDisclosure label="RESUMEN CON SPOILERS">
            <p>{title.event}</p>
          </SpoilerDisclosure>
        </ProgressSpoilerGate>
      </div>
      <dl className="title-dossier-meta" data-reveal>
        <div>
          <dt>{details ? "ESTRENO" : "POSICIÓN"}</dt>
          <dd>{details?.releaseDate ?? title.period}</dd>
        </div>
        <div>
          <dt>{details ? "DURACIÓN" : "FORMATO"}</dt>
          <dd>{details?.runtime ?? title.type}</dd>
        </div>
        <div>
          <dt>{details ? "CLASIFICACIÓN" : "ETAPA"}</dt>
          <dd>{details?.certification ?? title.phase}</dd>
        </div>
        <div>
          <dt>{details ? "DISPONIBILIDAD" : "CONTINUIDAD"}</dt>
          <dd>{details ? `${details.availability} · CONSULTAR SEGÚN REGIÓN` : title.continuity}</dd>
        </div>
      </dl>
    </section>
  );
