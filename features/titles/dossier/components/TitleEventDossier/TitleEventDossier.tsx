...
export function TitleEventDossier({ title, details }: { title: TitleDossier; details?: TitleDetails }) {
  return <section className="title-dossier profile-section">
    ...
    <div className="title-dossier-event" data-reveal><ProgressSpoilerGate requirement={{ allOf: [title.slug] }}><SpoilerDisclosure label="RESUMEN CON SPOILERS"><p>{title.event}</p></SpoilerDisclosure></ProgressSpoilerGate></div>
