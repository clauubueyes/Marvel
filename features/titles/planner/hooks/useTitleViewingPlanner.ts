...
import {
  addPlanToGoogleCalendar,
  deleteNexusGoogleCalendar,
...
export function useTitleViewingPlanner(titles: PlannerTitle[]) {
  ...
  const subscribeCalendar = useCallback(
    (callback: () => void) => subscribeGoogleCalendar(callback),
    [],
  );
  const syncedCalendarId = useSyncExternalStore(
    subscribeCalendar,
    googleCalendarSnapshot,
    () => "",
  );
  ...
  const plan = useMemo(
    () =>
      createViewingPlan(activeTitles, { startDate, weekDays, titlesPerWeek, startTime, endTime }),
    [activeTitles, endTime, startDate, startTime, titlesPerWeek, weekDays],
  );
  ...
  function downloadPlan() {
    ...
    const blob = new Blob([createIcsCalendar("Mi selección", absolutePlan)], {
  ...
  function replaceGoogleCalendarPlan() {
    if (
      !window.confirm(
        "Se eliminará el calendario NEXUS actual y se creará de nuevo con este plan. ¿Continuar?",
      )
    )
      return;
    void authorizeGoogle(async (accessToken) => {
      setGoogleStatus("syncing");
      setGoogleMessage("Reemplazando el plan anterior…");
      setGoogleProgress(0);
      const result = await replaceNexusGoogleCalendar(accessToken, plan, setGoogleProgress);
      setGoogleStatus("done");
      setGoogleMessage(`Plan actualizado: ${result.created} sesiones sincronizadas.`);
    });
  }
  ...
  function removeGoogleCalendar() {
    if (
      !window.confirm(
        "Se eliminará de Google Calendar el calendario NEXUS y todas sus sesiones. Esta acción no se puede deshacer. ¿Continuar?",
      )
    )
      return;
    void authorizeGoogle(async (accessToken) => {
      setGoogleStatus("syncing");
      setGoogleMessage("Eliminando el calendario NEXUS…");
      await deleteNexusGoogleCalendar(accessToken);
      setGoogleStatus("done");
      setGoogleMessage("El calendario NEXUS se ha eliminado de Google Calendar.");
    });
  }
