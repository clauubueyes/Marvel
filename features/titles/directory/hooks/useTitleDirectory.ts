...
import {
  TITLE_PROGRESS_EVENT,
  TITLE_PROGRESS_STORAGE_KEY,
  type TitleOrderMode,
...
function canPlanTitle(title: TitleDirectoryEntry) {
  return (
    title.coverage !== "TÍTULO ANUNCIADO" &&
    title.type !== "ONE-SHOT" &&
    !/CORTOS/i.test(title.runtime)
  );
}

export function useTitleDirectory(
  titles: TitleDirectoryEntry[],
  initial: TitleDirectoryInitialFilters = {},
) {
  ...
  const [progressFilter, setProgressFilter] = useState<TitleProgressFilter>(
    initial.progressFilter ?? "TODOS",
  );
  ...
  const {
    values: watched,
    setMany: setTitlesWatched,
    toggle: toggleTitleWatched,
    ready: progressReady,
  } = useMovieProgress({
    ...
    return titles
      .filter((title) => {
        const matchesQuery =
          !normalizedQuery ||
          normalizeSearchText(
            `${title.title} ${title.event} ${title.phase} ${title.period}`,
          ).includes(normalizedQuery);
        const matchesProgress =
          progressFilter === "TODOS" ||
          (progressFilter === "VISTOS" ? watched.has(title.slug) : !watched.has(title.slug));
        return (
          matchesQuery &&
          (type === "TODOS" || title.type === type) &&
          (phase === "TODAS" || title.phase === phase) &&
          (saga === "TODAS" || getTitleSaga(title.phase) === saga) &&
      ...
      .sort((left, right) => {
        ...
        if (sortMode === "AÑO" || orderMode === "ESTRENO")
          return (
            left.releaseDateISO.localeCompare(right.releaseDateISO) || left.order - right.order
          );
  ...
  const pendingTitles = useMemo(
    () => titles.filter((title) => !watched.has(title.slug) && canPlanTitle(title)),
    [titles, watched],
  );
  const visiblePending = useMemo(
    () => visibleTitles.filter((title) => !watched.has(title.slug) && canPlanTitle(title)),
    [visibleTitles, watched],
  );
  const plannerTitles = useMemo(
    () =>
      titles
        .filter((title) => selectedTitles.has(title.slug))
        .map((title) => ({
          id: title.slug,
          title: title.title,
  ...
  function selectForPlan(entries: TitleDirectoryEntry[]) {
    setSelectedTitles(
      new Set(
        entries
          .filter((title) => !watched.has(title.slug) && title.coverage !== "TÍTULO ANUNCIADO")
          .map(({ slug }) => slug),
      ),
    );
    setPlanning(true);
