    ...
    <section className="title-library-toolbar" aria-label="Gestión de la biblioteca">
      <label className="title-search">
        ...
        <input
          ...
          placeholder="Título, acontecimiento, año…"
        />
      </label>
      <div className="title-library-progress">
        <strong>
          {watched.size}
          <small>/ {titles.length}</small>
        </strong>
        <span>VISTOS</span>
        <i>
          <b style={{ width: `${Math.round((watched.size / titles.length) * 100)}%` }} />
        </i>
      </div>
      <button
        className="title-primary-plan"
        type="button"
        onClick={() => selectForPlan(pendingTitles)}
      >
        PLANIFICAR <b>{pendingTitles.length}</b>
      </button>
    </section>
  );
}
