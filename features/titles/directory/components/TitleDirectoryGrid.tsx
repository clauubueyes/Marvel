...
export function TitleDirectoryGrid({
  titles,
  directory,
  onToggleWatched,
}: {
  ...
  return (
    <section
      className={`titles-directory ${directory.viewMode === "CUADRÍCULA" ? "is-grid" : "is-list"}`}
    ...
    >
      ...
      {!titles.length && (
        <div className="title-directory-empty">
          ...
          <button onClick={directory.resetFilters}>MOSTRAR TODO</button>
        </div>
      )}
      {titles.length > visibleCount && (
        <button
          ref={loadMoreRef}
          type="button"
          className="title-load-more"
          onClick={() => setVisibleCount((count) => count + VISIBLE_COUNT_STEP)}
        >
          CARGAR MÁS · <b>{titles.length - visibleCount}</b> RESTANTES
        </button>
      )}
    </section>
  );
