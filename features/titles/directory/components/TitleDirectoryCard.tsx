...
export function TitleDirectoryCard({ title, viewMode, planning, isSelected, isWatched, onToggleSelected, onToggleWatched }: TitleDirectoryCardProps) {
  return <div className={`title-directory-row ${planning ? "is-planning" : ""} ${isSelected ? "is-selected" : ""} ${isWatched ? "is-watched" : ""}`}>
    ...
    <div className="title-card-actions">
      <button type="button" className="title-watch-toggle" onClick={onToggleWatched} aria-pressed={isWatched}>{isWatched ? "✓ VISTO" : "MARCAR VISTO"}</button>
