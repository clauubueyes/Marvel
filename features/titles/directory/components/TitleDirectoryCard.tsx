...
export const TitleDirectoryCard = memo(function TitleDirectoryCard({ title, viewMode, planning, isSelected, isWatched, progressReady, onToggleSelected, onToggleWatched }: TitleDirectoryCardProps) {
  return <div className={`title-directory-row ${planning ? "is-planning" : ""} ${isSelected ? "is-selected" : ""} ${isWatched ? "is-watched" : ""}`}>
    <Link className="title-directory-card" href={`/titulos/${title.slug}`} data-reveal>
      <div className="title-card-art"><Image src={getTitleImage(title.slug)} alt={`Cartel de ${title.title}`} fill sizes={viewMode === "CUADRÍCULA" ? "(max-width: 700px) 90vw, (max-width: 1150px) 45vw, (max-width: 1450px) 30vw, 24vw" : "180px"} /><span>{isWatched ? "✓" : String(title.order).padStart(2, "0")}</span><small>{title.type}</small></div>
    ...
    <div className="title-card-actions">
      <button type="button" className="title-watch-toggle" disabled={!progressReady} onClick={() => onToggleWatched(title)} aria-pressed={isWatched}>{isWatched ? "✓ VISTO" : "MARCAR VISTO"}</button>
      {planning && <button type="button" className="title-plan-select" onClick={() => onToggleSelected(title.slug)} aria-pressed={isSelected}><span>{isSelected ? "✓" : "+"}</span>{isSelected ? "EN EL PLAN" : "AÑADIR AL PLAN"}</button>}
    </div>
  </div>;
});
