import { TITLE_CONTINUITIES, TITLE_SAGAS, TITLE_TYPES, type TitleProgressFilter, type TitleSortMode } from "@/constants/titleDirectory";
...
export function TitleFilters({ directory }: { directory: ReturnTypeOfUseTitleDirectory }) {
  return <section className="title-filters" aria-label="Filtros del catálogo de títulos">
    ...
    <div className="title-advanced-filters">
      ...
      <button className="filter-reset" onClick={directory.resetFilters}>RESTABLECER</button>
    </div>
    {(directory.query || directory.type !== "TODOS" || directory.phase !== "TODAS" || directory.saga !== "TODAS" || directory.continuity !== "TODAS" || directory.progressFilter !== "TODOS") && <div className="title-active-filters" aria-label="Filtros aplicados">
      {directory.query && <button type="button" onClick={() => directory.setQuery("")}>BÚSQUEDA · {directory.query} ×</button>}
      {directory.type !== "TODOS" && <button type="button" onClick={() => directory.setType("TODOS")}>TIPO · {directory.type} ×</button>}
      {directory.phase !== "TODAS" && <button type="button" onClick={() => directory.setPhase("TODAS")}>FASE · {directory.phase} ×</button>}
      {directory.saga !== "TODAS" && <button type="button" onClick={() => directory.setSaga("TODAS")}>SAGA · {directory.saga} ×</button>}
      {directory.continuity !== "TODAS" && <button type="button" onClick={() => directory.setContinuity("TODAS")}>CONTINUIDAD · {directory.continuity} ×</button>}
      {directory.progressFilter !== "TODOS" && <button type="button" onClick={() => directory.setProgressFilter("TODOS")}>ESTADO · {directory.progressFilter} ×</button>}
    </div>}
  </section>;
}
