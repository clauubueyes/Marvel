...
export function EntityHero({ entity, imageTitle }: { entity: MCUEntity; imageTitle?: MCUEntry }) {
  return (
    <section className="entity-profile-hero">
      {imageTitle && (
        <div className="entity-profile-art" aria-hidden="true">
          <Image
            src={`/api/title-image?title=${encodeURIComponent(imageTitle.title)}&type=${encodeURIComponent(imageTitle.type)}`}
            alt=""
            fill
            priority
            sizes="60vw"
