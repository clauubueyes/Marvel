...
export function SpoilerProgressSettings({ titles, characters }: Props) {
  ...
  return <section id="spoilers" className="account-spoilers" aria-labelledby="spoilers-heading">
    ...
    <p className="account-message" role="status">{!progress.ready ? "Recuperando progreso…" : pending ? "Guardando cambios…" : `${progress.values.size} títulos vistos.`} {user ? "Se guarda en tu cuenta." : "Como invitado, se usa el progreso de títulos de este navegador."}</p>
