  ...
  return (
    <div className="account-form">
      <label className="account-checkbox">
        <input
          type="checkbox"
          checked={user?.user_metadata?.avoid_spoilers !== false}
          disabled={busy}
          onChange={(event) => void change(event.target.checked)}
          aria-describedby="spoiler-preference-hint"
        />
        <span>Evitar spoilers</span>
      </label>
      <small id="spoiler-preference-hint">
        {user?.user_metadata?.avoid_spoilers === false
          ? "Ves el contenido con spoilers. Tu progreso se conserva si vuelves a activar la protección."
          : "El contenido protegido se adapta a las obras que has visto."}
      </small>
