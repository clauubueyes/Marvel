...
export function CharacterDirectory({ characters }: { characters: Character[] }) {
  ...
  return (
    <>
      ...
      />
      <section
        className="characters-directory"
        key={`${filters.category}-${filters.status}-${filters.universe}-${filters.affiliation}`}
        aria-label="Catálogo de personajes"
        aria-live="polite"
      >
        {filters.visibleCharacters.map((character, index) => (
          <CharacterDirectoryCard character={character} index={index} key={character.id} />
        ))}
        {!filters.visibleCharacters.length && (
          <div className="character-empty">
            <strong>SIN COINCIDENCIAS</strong>
            <p>Prueba otra combinación de filtros.</p>
            <button onClick={filters.reset}>MOSTRAR TODO</button>
          </div>
        )}
      </section>
    </>
