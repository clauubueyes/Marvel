...
const categories: CharacterCategoryFilter[] = [
  "TODOS",
  "HÉROE",
  "ANTI-HÉROE",
  "VILLANO",
    ...
    <section className="character-filters" aria-label="Filtros de personajes">
      <div>
        ...
        {categories.map((value) => (
          <button
            key={value}
            className={props.category === value ? "active" : ""}
      ...
      <label>
        ...
        <select
          value={props.status}
          onChange={(event) => props.setStatus(event.target.value as CharacterStatusFilter)}
        >
          <option>TODOS</option>
      ...
      <label>
        <span>UNIVERSO</span>
        <select value={props.universe} onChange={(event) => props.setUniverse(event.target.value)}>
          <option>TODOS</option>
          {props.universes.map((value) => (
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) => props.setAffiliation(event.target.value)}
        >
          <option>TODAS</option>
          {props.affiliations.map((value) => (
            <option key={value}>{value}</option>
          ))}
        </select>
      </label>
      <p>
        <strong>{props.resultCount}</strong> RESULTADOS
      </p>
      <button className="filter-reset" onClick={props.onReset}>
        LIMPIAR
      </button>
    </section>
  );
