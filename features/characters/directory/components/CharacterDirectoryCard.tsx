...
export function CharacterDirectoryCard({ character, index }: { character: Character; index: number }) {
  return <div className="character-directory-card" data-reveal data-tilt style={{ "--card-accent": character.color, "--delay": `${Math.min(index, 11) * 45}ms` } as React.CSSProperties}>
    ...
    <Link className="character-card-link" href={`/personajes/${character.id}`} aria-label={`Abrir expediente de ${character.name}`} />
