...
export function CharacterFilmography({ appearances }: { appearances: Character["appearances"] }) {
  ...
  const visibleAppearances = appearances.filter(({ titleId }) =>
    canRevealSpoiler({ allOf: [titleId] }, progress),
  );
  if (!visibleAppearances.length) return null;
  return (
    <section className="filmography profile-section" data-scroll-section data-section-index="05">
      <div className="film-heading" data-reveal>
        <p className="section-label">05 / EN PANTALLA</p>
        <h2>
          SU HISTORIA
          <br />
          <em>EN CINE</em>
        </h2>
        <p>Una selección de los capítulos que definieron su recorrido audiovisual.</p>
      </div>
      <div className="film-reel" data-reveal>
        {visibleAppearances.map((appearance, index) => (
          <Link href={`/titulos/${appearance.titleId}`} key={appearance.title}>
