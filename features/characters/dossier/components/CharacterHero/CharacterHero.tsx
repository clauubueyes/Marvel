...
export function CharacterHero({ character, motion }: CharacterHeroProps) {
  ...
  return (
    <section className="profile-hero">
      ...
      <div className="profile-title profile-title-arrival">
        ...
        <h1 aria-label={character.name}>
          {nameLines.map((line, lineIndex) => (
            <span className="profile-name-line" aria-hidden="true" key={`${line}-${lineIndex}`}>
              {Array.from(line).map((letter, letterIndex) => (
                <span
                  className="profile-name-letter"
                  style={{ "--letter": letterIndex } as React.CSSProperties}
      ...
      <div
        className={`profile-figure profile-arrival profile-arrival-${character.id}`}
        aria-hidden="true"
      >
        <div className="arrival-trail">
          <i />
          <i />
          <i />
          <i />
