...
export function CharacterStory({
  ...
  const visibleActs = sourceActs.filter((act) =>
    canRevealSpoiler(act.chapter.spoiler ?? UNREVIEWED_SPOILER, progress),
  );
  ...
  if (hasMore)
    acts.push({
      preview: true,
      numeral: "▶",
      label: "TU PRÓXIMA HISTORIA",
      chapter: {
        year: "CONTINÚA",
        kicker: `SIGUE A ${characterName}`,
        title: next?.title ?? "Tu historia continúa",
        text: !progress.ready
          ? "Preparando tu recorrido según lo que has visto."
          : next
            ? "Este es tu siguiente título pendiente. Descubre su tráiler y vuelve después de verlo para continuar la historia."
            : "No quedan títulos estrenados pendientes en el recorrido de este personaje. Su historia continuará aquí.",
      },
    });
      ...
      (context) => {
        ...
        const setAct = (index: number): void => {
          ...
          cards.forEach((card, i) =>
            card.setAttribute("data-active", i === act ? "true" : "false"),
          );
          ...
          section
            .querySelectorAll<HTMLElement>(".story-year-storyline b")
            .forEach((yearEl, yearIndex) => {
              const state = yearIndex <= act ? "true" : "false";
              if (yearEl.getAttribute("data-active") !== state)
                yearEl.setAttribute("data-active", state);
            });
        };

        setAct(
          cards.findLastIndex((card) => card.getBoundingClientRect().top <= innerHeight * 0.4),
        );
          ...
          if (card.hasAttribute("data-next-watch") && clip && image) {
            ...
            tl.fromTo(
              ...
              .fromTo(
                ...
                { scale: 1.02, y: index % 2 === 0 ? -30 : 30, rotate: 0, duration: 0.14 },
                0,
              )
              .fromTo(
                img,
                { scale: 1.18, x: dir * 34, filter: "blur(6px)" },
                { scale: 1, x: 0, filter: "blur(0px)", duration: 0.16 },
                0.02,
              );
          }
    ...
    <>
      ...
      {acts.length > 0 && (
        <section
          ...
          data-history
          data-scroll-section
          data-section-index="HISTORIA"
          aria-label={`Historia de ${characterName}`}
        >
          <div className="story-images">
            <div className="story-image-base" aria-hidden="true">
              {!acts[0]?.preview && (acts[0]?.image || portrait) ? (
                <Image
                  src={acts[0]?.image || portrait!}
                  alt=""
                  fill
                  sizes="(max-width: 900px) 100vw, 47vw"
                  style={{ objectPosition: acts[0]?.image ? "center" : portraitPosition }}
                />
              ) : null}
            </div>
            {acts.map((act, index) => (
              <div
                className={`story-image${act.preview ? " story-trailer-image" : ""}`}
                aria-hidden={act.preview ? activeIndex !== index : true}
                inert={act.preview && activeIndex !== index}
                key={`${act.numeral}-${act.chapter.year}`}
              >
                <div className="story-image-clip" data-index={index}>
                  {act.preview ? (
                    <div className="story-trailer-stage">
                      {next?.trailerId ? (
                        <NextTrailer
                          key={`${next.slug}:${activeIndex === index}`}
                          slug={next.slug}
                          title={next.title}
                          videoId={next.trailerId}
                          active={activeIndex === index}
                        />
                      ) : (
                        <p>
                          {next ? "Tráiler no disponible en el catálogo" : "La historia continuará"}
                        </p>
                      )}
                    </div>
                  ) : act.image || portrait ? (
                    <Image
                      src={act.image || portrait!}
                      alt=""
                      fill
                      sizes="(max-width: 900px) 100vw, 47vw"
                      style={{ objectPosition: act.image ? "center" : portraitPosition }}
                    />
                  ) : null}
            ...
            <em className="story-film-tag" aria-hidden="true">
              NEXUS ARCHIVE · CINE CÓSMICO
            </em>
          </div>
          ...
          <div className="story-track">
            {acts.map((act, index) => {
              ...
              return (
                <article
                  className={`story-step story-card${act.preview ? " story-next-card" : ""}`}
                  data-next-watch={act.preview || undefined}
                  key={act.numeral}
                  data-mood={mood}
                  data-index={index}
                  data-active={index === 0 ? "true" : "false"}
                  data-history-step={act.preview ? undefined : true}
                  aria-label={`${act.label} · ${act.chapter.title}`}
                  aria-live="polite"
                >
                        ...
                        <Fragment key={wordIndex}>
                          ...
                          <span className="story-title-word" aria-hidden="true">
                            {Array.from(word).map((letter, letterIndex) => (
                              <i className="story-title-letter" key={letterIndex}>
                                {letter}
                              </i>
                            ))}
                          </span>
                  ...
                  <div className="story-card-foot">
                    {act.preview ? (
                      <div className="next-watch-actions">
                        <Link
                          className="story-chip"
                          href={next ? `/titulos/${next.slug}` : "/titulos"}
                        >
                          EXPLORAR TÍTULO ↗
                        </Link>
                        <Link className="story-chip" href="/titulos">
                          ACTUALIZAR LO QUE HE VISTO ↗
                        </Link>
                      </div>
                    ) : (
