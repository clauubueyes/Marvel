...
export function CharacterFacts({
  ...
  requirements?: SpoilerRequirement[];
}) {
  const progress = useSpoilerProgress();
  const facts = sourceFacts.filter((_, index) =>
    canRevealSpoiler(requirements?.[index] ?? UNREVIEWED_SPOILER, progress),
  );
  if (!facts.length) return null;
  return (
    <section className="profile-facts profile-section" data-scroll-section data-section-index="04">
      <div className="facts-heading" data-reveal>
        <p className="section-label">04 / DATOS CURIOSOS</p>
        <h2>
          LO QUE NO
          <br />
          <em>SE VE</em>
        </h2>
        <span>LOS DETALLES QUE YA PUEDES EXPLORAR EN SU HISTORIA.</span>
      </div>
