...
export function CharacterFacts({ facts: sourceFacts, requirements }: { facts: Character["facts"]; requirements?: SpoilerRequirement[] }) {
  ...
  const facts = sourceFacts.map((fact, index) => {
    const requirement = requirements?.[index] ?? UNREVIEWED_SPOILER;
    const hint = spoilerProgressHint(requirement, progress);
    return protectContent(fact, requirement, progress, {
      value: hint ? `${hint.watched}/${hint.required}` : "🔒",
      label: "Contenido bloqueado por spoilers",
