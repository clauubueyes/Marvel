        ...
        <label className="search-command">
          ...
          <div>
            <input
              ...
              placeholder="Ej. Wanda, incursiones, Wakanda…"
      ...
      <section className="search-results" aria-live="polite">
        <div className="search-result-tools">
          <div role="group" aria-label="Filtrar resultados">
            {filters.map((item) => (
              <button
                className={search.filter === item ? "active" : ""}
                type="button"
                key={item}
                onClick={() => search.setFilter(item)}
              >
                {item} {item !== "TODO" && <span>{search.counts[item]}</span>}
              </button>
            ))}
          </div>
          <p>
            {search.deferredQuery.trim() ? (
              <>
                <b>{search.allResults.length}</b> RESULTADOS PARA “{search.deferredQuery.trim()}”
              </>
            ) : (
