...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return route
    ? createPageMetadata({
        title: `${route.name} — Ruta NEXUS`,
        socialTitle: route.name,
        description: route.description,
        path: `/rutas/${route.slug}`,
      })
    : { title: "Ruta no encontrada — NEXUS" };
}
...
export default async function RoutePage({ params }: PageProps) {
  ...
  return (
    <main
      ...
      style={{ "--accent": route.accent, "--accent-2": "#4f6b28" } as React.CSSProperties}
    >
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "ItemList",
            url: new URL(`/rutas/${route.slug}`, siteConfig.url).toString(),
            name: route.name,
            description: route.description,
            numberOfItems: route.steps.length,
            itemListElement: route.steps.map((step, index) => ({
              "@type": "ListItem",
              position: index + 1,
              url: new URL(`/titulos/${step.titleId}`, siteConfig.url).toString(),
            })),
          }).replaceAll("<", "\\u003c"),
        }}
      />
      <section
        className="route-profile-hero"
      ...
      >
        ...
        <div>
          <span>{String(route.steps.length).padStart(2, "0")} CAPÍTULOS</span>
          <span>{formatRouteDuration(route.estimatedMinutes)}</span>
