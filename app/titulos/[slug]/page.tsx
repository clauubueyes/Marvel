...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return title
    ? createPageMetadata({
        title: `${title.title} — Archivo NEXUS`,
        socialTitle: title.title,
        description: details?.spoilerFreeSynopsis ?? title.event,
        path: `/titulos/${title.slug}`,
      })
...
export default async function TitlePage({ params }: PageProps) {
  ...
  const beforeTitles =
    details?.watchBefore.flatMap((slug) => {
      const related = getTitle(slug);
      return related ? [related] : [];
    }) ?? [];
  const afterTitles =
    details?.watchAfter.flatMap((slug) => {
      const related = getTitle(slug);
      return related ? [related] : [];
    }) ?? [];
