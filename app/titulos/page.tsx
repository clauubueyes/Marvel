...
export const metadata = createPageMetadata({
  ...
  description:
    "Archivo cronológico de películas, series, especiales y one-shots del universo audiovisual de Marvel.",
  path: "/titulos",
});
...
export default async function TitlesPage({ searchParams }: PageProps) {
  ...
  const titles: TitleDirectoryEntry[] = mcuCatalog.map((title) => ({
    ...
    routes: viewingRoutes
      .filter(({ steps }) => steps.some(({ titleId }) => titleId === title.slug))
  ...
  return (
    ...
    >
      ...
      <section className="titles-index-hero">
        ...
        <p>
          Explora el archivo completo y descubre qué sucede en cada título, dónde encaja y qué
          personajes conecta.
        </p>
        <div>
          <strong>{String(mcuCatalog.length).padStart(2, "0")}</strong>
          <span>
            TÍTULOS DOCUMENTADOS · {getDetailedTitleIds().length} EXPEDIENTES COMPLETOS O ANUNCIADOS
          </span>
        </div>
      </section>
