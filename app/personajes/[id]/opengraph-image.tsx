...
export default async function CharacterOpenGraphImage({ params }: { params: Promise<{ id: string }> }) { 
    const character = getCharacter((await params).id); 
    return createSocialImage({ 
        kicker: character ? `GUÍA MARVEL · NEXUS · ${character.role}` : "ARCHIVO DE PERSONAJES", 
        title: character?.name ?? "Personaje no encontrado", 
        subtitle: character?.description ?? "Expediente NEXUS", 
