...
if (!issues.length) {
  console.log(
    `Auditoría editorial superada sin observaciones. ${reviewedChronologyCount} diferencias cronológicas revisadas.`,
  );
} else {
  for (const severity of ["ERROR", "AVISO", "INFO"] as const) {
    ...
    group.forEach(({ titleId, field, message }) =>
      console.log(`- ${titleId} · ${field}: ${message}`),
    );
  }
  console.log(
