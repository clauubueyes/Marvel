...
const staticImages = new Map<string, string>([
  ...characters
    ...
    .map(({ id, image }) => [`characters/${id}`, image] as const),
...
async function main() {
  ...
  for (let index = 0; index < mcuCatalog.length; index += 4) {
    await Promise.all(
      mcuCatalog.slice(index, index + 4).map(async ({ slug, title, type }) => {
  ...
  const videoImages = [
    ...characters.map(
      ({ id, image, screenMoment }) => [`moments/${id}`, screenMoment.videoId, image] as const,
    ),
  ...
  for (let index = 0; index < videoImages.length; index += 6) {
    await Promise.all(
      videoImages.slice(index, index + 6).map(async ([name, videoId, fallback]) => {
        try {
          ...
          try {
            ...
            const output = path.join(process.cwd(), "public", `${name}.webp`);
            await mkdir(path.dirname(output), { recursive: true });
            await sharp(path.join(process.cwd(), "public", fallback.replace(/^\//, "")))
              .webp({ quality: 82 })
              .toFile(output);
          }
        }
