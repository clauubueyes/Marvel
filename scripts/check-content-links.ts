    ...
    ? [
        {
          titleId,
          label: "TRÁILER",
          url: `https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${details.trailerId}&format=json`,
        },
      ]
    : [];
...
async function checkLink(target: LinkTarget): Promise<LinkResult> {
  ...
  try {
    ...
    return {
      ...
      ok: response.ok || response.status === 403 || response.status === 429,
    };
  } catch (error) {
    return {
      ...target,
...
async function main() {
  ...
  if (failures.length) {
    ...
    failures.forEach(({ titleId, label, url, status, error }) =>
      console.error(`- ${titleId} · ${label}: ${status || error} · ${url}`),
    );
    process.exitCode = 1;
  } else {
